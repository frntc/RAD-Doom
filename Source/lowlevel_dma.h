...
__attribute__( ( always_inline ) ) inline  
void emuPrepareManyWrites( register u32 &g2 )
{
	CLR_GPIO( bDIR_Dx );
	SET_GPIO( bGAME_OUT );
	SET_GPIO( bRW_OUT );
...
__attribute__( ( always_inline ) ) inline  
void emuEndManyWrites( register u32 &g2 )
{
...
__attribute__( ( always_inline ) ) inline  
void emuWriteByteMany_p1( register u32 &g2, u16 addr, u8 data )
{
	register u32 DD = flipByte( ( addr ) & 255 ) << D0;
	SET_GPIO( DD );
...
//		WAIT_UP_TO_CYCLE( reu.TIMING_READ_BA_WRITING );			
	...
	WAIT_UP_TO_CYCLE(reu.TIMING_ENABLE_RWOUT_ADDR_LATCH_WRITING - 40 ); // new VIC works without this wait
//	OUT_GPIO( RW_OUT );
	CLR_GPIO( bRW_OUT );
...
__attribute__( ( always_inline ) ) inline  
void emuWriteByteMany_p2( register u32 &g2, bool releaseDMA )
{
	WAIT_FOR_VIC_HALFCYCLE
	RESTART_CYCLE_COUNTER
